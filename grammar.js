...
module.exports = grammar({
  ...
  rules: {
    ...
    function_definition: ($) =>
      prec.left(
        1,
        seq(field("name", $.variable), "=", field("value", $.function_block)),
      ),
      ...
      prec.right(
        choice(
          choice(
            ...
            seq($.class, seq("(", optional($.parameter_call_list), ")")),
          ),
    ...
    code_block: ($) => seq("(", optional($._expression_sequence), ")"),
    ...
    parameter_list: ($) =>
      choice(
        ...
        seq(
          "|",
          sepBy(
            choice($._space_separator, ","),
            choice($.argument, alias($.variable_argument, $.argument)),
          ),
          "|",
        ),
      ),
    ...
    argument_calls: ($) => choice($.named_argument, $.unnamed_argument),
      ...
      prec.left(
        ...
        field(
          ...
          seq(
            ...
            seq(choice("=", ":"), choice($.function_call, $._object)),
          ),
        ),
      ),
    ...
    symbol: ($) =>
      choice(
        prec.left(
          seq(
            "\\",
            optional(choice($.identifier, /[0-9]+/, $.escape_sequence)),
          ),
        ),
    ...
    return_statement: ($) =>
      prec.left(seq("^", choice($._object, $.function_call))),

    // Definition of class
    class_def: ($) =>
      prec(
        PRECEDENCE.class_def,
        seq(
          optional("+"),
          $.class,
          optional(seq(":", alias($.class, $.parent_class))),
          "{",
          repeat(
            choice(
              // Variables
              ...
              // Instance method
              seq(
                alias($.identifier, $.instance_method_name),
                $.function_block,
              ),
        ...
        seq(
          ...
          choice(
            ...
            prec.left(
              seq(
                optional(choice("#", $.ref)),
                optional(choice(optional(alias($.class, $.collection_type)))),
                "[",
    ...
    _collection_sequence: ($) =>
      seq(sepBy1(",", choice($.associative_item, $._object)), optional(",")),

    _paired_associative_sequence: ($) =>
      seq(sepBy1(",", $.associative_item), optional(",")),
    ...
    _collection_types: ($) =>
      choice($._unordered_collection_types, $._ordered_collection_types),
    _unordered_collection_types: ($) =>
      choice(
        "Bag",
        "Dictionary",
        "Environment",
    ...
    indexed_collection: ($) =>
      seq($._indexable_object, repeat($._index), $._index),
    ...
    binary_expression: ($) => {
      const table = [
        ...
        // @TODO
        [
          PRECEDENCE.selectorBinary,
          alias(/(r#)?[a-zA-Zα-ωΑ-Ωµ_][a-zA-Zα-ωΑ-Ωµ\d_]*:/, $.method_name),
        ],
    ...
    unary_expression: ($) => {
      ...
      return choice(
        ...table.map(([precedence, operator]) =>
          prec.left(
            precedence,
            seq(field("operator", operator), field("right", choice($._object))),
          ),
        ),
      );
    },
    ...
    nil_check: ($) =>
      prec.left(seq($._object, choice("?", "!?", "??"), $._object)),
    ...
    control_structure: ($) =>
      prec(
        PRECEDENCE.controlstruct,
        choice($.if, $.while, $.for, $.forby, $.case, $.switch),
      ),
      ...
      choice(
        ...
        prec.right(
          seq(
            field("name", "if"),
            "(",
            field("expression", prec.left(choice($.function_call, $._object))),
            ")",
      ...
      choice(
        ...
        prec.right(
          seq(
            ...
            $.code_block,
            // "(", $._object, ")",
            repeat(seq(choice($.function_call, $._object), $.function_block)),
            seq(choice($.function_call, $._object), $.function_block),
          ),
        ),
      ),
  },
